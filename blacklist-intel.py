    ...
    for line in cvsfile:
        if line.startswith('# Listing') or lineCount >= 1:
            ...
            if sslComp.__len__() <= 1:
                continue
            if lineCount == 0:
                writeFile.write(u'#fields\tindicator\tindicator_type\tmeta.url\tmeta.source\tmeta.do_notice\n')
                lineCount += 1
            else:
                writeFile.write(
                    sslComp[1] + '\tIntel::FILE_HASH' + '\thttps://sslbl.abuse.ch/ssl-certificates/sha1/' \
                    + sslComp[1] + '\tabuse.ch\tT\n')
                lineCount += 1
    ...
    for line in cvsfile:
        if line.startswith('# ja3') or lineCount >= 1:
            ...
            if ja3Comp.__len__() <= 1:
                continue
            if lineCount == 0:
                ja3writeFile.write(u'#fields\tindicator\tindicator_type\tmeta.url\tmeta.source\tmeta.do_notice\n')
                lineCount += 1
            else:
                ja3writeFile.write(
                    ja3Comp[0] + '\tIntel::JA3' + '\thttps://sslbl.abuse.ch/ja3-fingerprints/' \
                    + ja3Comp[0] + '\tabuse.ch\tT\n')
                lineCount += 1
    ja3writeFile.close()
    ...
    for line in cvsfile:
        if line.startswith('# id') or lineCount >= 1:
            ...
            if urlComp.__len__() <= 1:
                continue
            if lineCount == 0:
                urlHausWriteFile.write(u'#fields\tindicator\tindicator_type\tmeta.url\tmeta.source\tmeta.do_notice\n')
                lineCount += 1
            else:
                onlyURL = urlComp[2].split('://')[1]
                urlHausWriteFile.write(
                    onlyURL + '\tIntel::URL' + '\t'+urlComp[-2] + '\tabuse.ch\tT\n')
                lineCount += 1
    ...
    for line in csvfile:
        if line.startswith('# Firstseen') or lineCount >= 1:
            ...
            if components.__len__() <= 1:
                continue
            if lineCount == 0:
                rwWriteFile.write(u'#fields\tindicator\tindicator_type\tmeta.url\tmeta.source\tmeta.do_notice\n')
                lineCount += 1
            else:
                domain = ''
